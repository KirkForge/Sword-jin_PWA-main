...
func show_victory(
	...
	xp_gained: int,
	gold_gained: int = 0,
	reward_weapon: String = "",
	reward_skill: String = "",
	stars: int = 1,
	ghost_time: float = -1.0,
	...
	if not _has_next_chapter:
		var act_title_path := (
			"res://assets/art/generated/act_titles/act%02d_title.webp" % GameState.current_act
		)
		...
		if loot_summary.total_drops > 0:
			...
			for r in rarity_order:
				if loot_summary.by_rarity.has(r):
					...
					loot_parts.append(
						"[color=%s]%s x%d[/color]" % [color_hex, label_text, data.count]
					)
	...
	if ghost_label:
		if ghost_time > 0 and completion_time > 0:
			...
			if time_diff < 0:
				...
				ghost_label.text = (
					"👻 GHOST: %.1fs | You beat it by %.1fs!" % [ghost_time, absf(time_diff)]
				)
				...
				ghost_label.text = (
					"👻 GHOST: %.1fs | Ghost was %.1fs faster" % [ghost_time, time_diff]
				)
	...
	if not _has_next_chapter:
		victory_bgm = (
			"bgm_triumphant" if AudioManager.bgm_cache.has("bgm_triumphant") else "bgm_victory"
		)
