...
func _ready():
	...
	if chapter_data.is_empty():
		...
		if Engine.has_singleton("ErrorScreen"):
			ErrorScreen.show_error(
				"Chapter Load Failed", "No chapter data available.\nTap Return to Menu to go back."
			)
		return
	...
	print(
		"Controls: WASD move | SPACE attack | LEFT SHIFT dodge | ESC pause | C chapter select | M mute"
	)
...
func _show_title_card(chapter_id: String) -> void:
	...
	if chapter_id in BOSS_CHAPTERS:
		...
		var boss_path: String = (
			"res://assets/art/generated/boss_splash/boss%02d_%s_portrait.webp"
			% [ch_idx, safe_title]
		)
...
func _get_bgm_for_current_chapter() -> String:
	...
	match act:
		...
		4:
			return "bgm_act4"
		5:
			return "bgm_siege" if _chapter_is_siege(chapter_id) else "bgm_act5"
		6:
			return "bgm_void" if _chapter_is_void(chapter_id) else "bgm_act6"
	return "bgm_battle"
...
func _update_ghost_hud():
	...
	if (
		not ghost_active
		or ghost_runner_instance == null
		or not is_instance_valid(ghost_runner_instance)
	):
		ghost_hud.text = ""
		return
...
func _update_combo_hud():
	...
	if p.last_hit_was_crit:
		...
		var color = (
			combo_colors[p.combo_count]
			if p.combo_count < combo_colors.size()
	...
	if "elite_patrol" in modifiers:
		for child in get_children():
			if child.is_in_group("enemy") and not child.is_dead:
				...
				child.health = child.max_health
				child.attack_damage = (
					int(child.attack_damage * 1.3) if child.get("attack_damage") != null else 12
				)
...
func _finish_chapter_complete():
	...
	var chapter_id_for_ghost: String = chapter_data.get(
		"chapter_id", "act%02d_ch%03d" % [GameState.current_act, GameState.current_chapter]
	)
	...
	var next_chapter_id_for_victory: String = (
		"" if next_chapter_id_raw == null else str(next_chapter_id_raw)
	)
